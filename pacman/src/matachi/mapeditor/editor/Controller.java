...
public class Controller implements ActionListener, GUIInformation  {
	...
	// Remove later
	private GameCallback gameCallback = new GameCallback("level");
	...
	public Controller() {
		...
		this.levelChecks.add(new PacManCheck());
//		this.levelChecks.add(new GoldCheck());
		this.levelChecks.add(new PortalCheck());
		this.levelChecks.add(new PillCheck());
	}

	public Controller(File map) {
		init(Constants.MAP_WIDTH, Constants.MAP_HEIGHT);
		this.levelChecks.add(new PacManCheck());
//		this.levelChecks.add(new GoldCheck());
		this.levelChecks.add(new PortalCheck());
		...
		if (e.getActionCommand().equals("flipGrid")) {
			...
			if(currFile!=null){
				for (LevelCheck checks : levelChecks) {
					check=checks.levelCheck(currFile, gameCallback);
					if(check==false){
						break;
					}
				}
				if(check!=false){
					ProgramAdapter adapter = new GameAdapter();
					try {
						adapter.loadMap(currFile,gameProperties);
						view.close();
	...
	private void saveFile() {
		...
		try {
			if (returnVal == JFileChooser.APPROVE_OPTION) {
				...
				paths.replace("\\", "/");
				for (LevelCheck checks : levelChecks) {
					checks.levelCheck(chooser.getSelectedFile(), gameCallback);
				}
				//currFile=chooser.getSelectedFile();
				currFile = new File(paths);
			}
	...
	public void loadFile(File map) {
		SAXBuilder builder = new SAXBuilder();
		try {
			BufferedReader in;
			FileReader reader = null;
			//File workingDirectory = new File(System.getProperty("user.dir"));
			File selectedFile=map;
			//chooser.setCurrentDirectory(workingDirectory);
			Document document;
			if (selectedFile.canRead() && selectedFile.exists()) {
